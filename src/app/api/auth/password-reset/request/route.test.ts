...
function request(body: unknown): NextRequest {
  return new NextRequest("http://localhost:8080/api/auth/password-reset/request", {
    method: "POST",
...
describe("POST /api/auth/password-reset/request", () => {
  ...
  it("passes the canonical email and preserved local destination to the service", async () => {
    const response = await POST(request({ email: "  USER@Example.COM ", next: "/settings/profile?tab=sessions#devices" }));

    expect(response.status).toBe(202);
    expect(mocks.requestPasswordReset).toHaveBeenCalledWith(
      "user@example.com",
      "/settings/profile?tab=sessions#devices",
    );
  });
  ...
  it("normalizes an external destination to the product root", async () => {
    const response = await POST(request({ email: "user@example.com", next: "https://attacker.example/settings" }));
  ...
  it("maps delivery failure to the existing unavailable response", async () => {
    mocks.requestPasswordReset.mockRejectedValue(new EmailDeliveryError());

    const response = await POST(request({ email: "user@example.com", next: "/settings/profile" }));
