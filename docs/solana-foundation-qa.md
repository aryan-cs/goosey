...
## Actual YES, NO and VOID settlement lifecycle

`npm run test:chain:resolution` passed **125 actual transactions** (99 successes, 26 expected rejections) on a fresh isolated ledger. Artifact SHA-256: `6222b66ef8964e902fee47acf5e9336dad58c0e1478dffd986954eff07a93523`; genesis `AzPjbJXZFPzeCL8XwkdqRniy7x2ycni4EwoshXksxEyx`; evidence `/tmp/goosey-solana-runner-ltZmdZ/`. This exact-artifact evidence supersedes neither earlier exchange runs nor their distinct hashes. No fabricated account state or Clock warp was used.

The initial resolution run exposed a stale ELF compiled before the finalized nine-account proposal ABI. It failed account validation rather than reaching reviewer authorization. Rebuilding current Rust produced the hash above and passed without weakening the expected rejection checks. Always rebuild after ABI edits and retain the loaded artifact hash; source-level builder tests alone cannot prove deployed compatibility.

Three markets used actual enrollment grants, SPL claims, deposits and CLOB fills. The run verified early close and pre-resolution-time rejection; draining outstanding orders before closure; frozen distinct reviewers; immutable proposal fingerprints; rejected VOID evidence followed by a new proposal sequence; prohibited post-resolution trading; YES/NO/VOID seat payouts; identical-wire and fresh-signature claim replay; and finalization blocked while holdings remain.

Six shipping nine-account finalized reader snapshots checked the **partially claimed Resolved phase**, not only final empty positions: YES had zero winning liability with three losing units left; NO retained 300,000 units of backing; odd-payout VOID retained 50,001 units after its first 50,000-unit payout. Final VOID dust was exactly one unit, moved to fee revenue only after all holdings were consumed. Another six reader snapshots verified Finalized state.
