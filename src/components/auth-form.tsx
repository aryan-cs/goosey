  ...
  return (
    <section className={`auth-card${register ? ` ${styles.registration}` : ""}`} aria-labelledby="auth-heading">
      ...
      <form onSubmit={submit}>
        ...
        <label><span>Email</span><div className="input-with-icon"><Mail /><input autoComplete="email" name="email" type="email" placeholder="you@example.com" required aria-describedby={register ? "email-hint" : undefined} /></div>{register && <small id="email-hint" className="field-hint">Use a real email address that you own, can access, and check regularly. We use it for verification, important account notices, and to contact you about any prize or payment for which you qualify.</small>}</label>
        ...
        {register ? <label className="checkbox-field"><input type="checkbox" name="acceptedCodeOfConduct" required /><span>I agree to the <Link href="/terms">Terms of Use</Link> and <Link href="/rules">community rules</Link>, acknowledge the <Link href="/privacy">Privacy Notice</Link>, and confirm that the email address I provided is accurate and under my control.</span></label> : null}
