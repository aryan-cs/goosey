...
export function SearchExperience({ initialQuery = "", syncUrl = true }: { initialQuery?: string; syncUrl?: boolean }) {
  ...
  return <div className="search-experience">
    ...
    {status === "ready" && count > 0 && <div className="search-results" key={`${results?.query}-${retry}`} aria-label={`Search results for ${results?.query}`}>
      ...
      {!!results?.profiles.length && <section><div className="search-group-heading"><h2>People</h2></div><div className="search-result-list">{results.profiles.map((profile, index) => <UserProfileLink className="search-result-row" style={{ animationDelay: `${Math.min((results.markets.length + results.events.length + index), 12) * 45}ms` }} username={profile.username} key={profile.id}><span className="search-result-icon"><UserRound aria-hidden="true" /></span><span><strong>{profile.displayName}</strong><small>@{profile.username}{profile.bio ? ` · ${profile.bio}` : ""}</small></span></UserProfileLink>)}</div></section>}
    </div>}
  </div>;
}
