  ...
  return (
    <section className="auth-card" aria-labelledby="auth-heading">
      ...
      <form onSubmit={submit}>
        ...
        <label><span>Password</span><div className="input-with-icon"><LockKeyhole /><input autoComplete={register ? "new-password" : "current-password"} name="password" type={showPassword ? "text" : "password"} required minLength={register ? 12 : undefined} /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff /> : <Eye />}</button></div></label>
        {!register ? <div className="auth-options"><span /><Link href="/reset-password">Forgot password?</Link></div> : null}
