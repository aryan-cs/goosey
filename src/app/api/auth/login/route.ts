...
import { emailVerificationState, loginUser, REGISTRATION_DEVICE_COOKIE_NAME, setSessionCookie } from "@/lib/auth";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await loginUser({
      ...
      ...(isValidRegistrationDeviceToken(registrationDeviceToken) ? { registrationDeviceToken } : {}),
