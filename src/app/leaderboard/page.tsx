...
export default async function LeaderboardPage() {
  ...
  return <div className="page-shell leaderboard-page"><header className="page-header"><span className="eyebrow">Hackathon standings</span><h1>Leaderboard</h1><p>Your rank uses your balance and what your open positions are worth. Starter feathers do not count.</p></header>{ranked.length ? <><LeaderboardPodium users={ranked.slice(0, 3)} /><section className="leaderboard-table" aria-label="Leaderboard standings">{ranked.map((user) => <LeaderboardRow user={user} key={user.id} />)}</section></> : <EmptyState title="No rankings yet" description="Rankings start after the first trade." />}</div>;
}
