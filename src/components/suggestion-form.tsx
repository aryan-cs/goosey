...
export function SuggestionForm() {
  ...
  return <form className="stacked-form" onSubmit={submit}><label>What should people predict?<input name="title" minLength={12} maxLength={180} required placeholder="Will...?" /></label><label>Category<select name="category" required defaultValue=""><option value="" disabled>Choose a category</option>{MARKET_CATEGORIES.filter((category) => category !== "Trending").map((category) => <option value={category} key={category}>{category}</option>)}</select></label><label>What should the options be and how should the market be resolved?<textarea name="description" minLength={30} maxLength={2000} rows={7} required placeholder="List the options people can bet on, what makes each option win, when trading closes, and how we can verify the result." /></label>{error && <p className="form-error" role="alert">{error}</p>}<button className="button button-primary" disabled={state === "sending"}>{state === "sending" && <LoaderCircle className="spin" />}Send suggestion</button></form>;
}
