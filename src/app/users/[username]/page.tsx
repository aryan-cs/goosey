...
export default async function UserProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const user = await db.user.findUnique({ where: { username: username.toLowerCase() }, include: { _count: { select: { trades: true, comments: { where: { status: "VISIBLE", market: { status: { not: "DRAFT" } } } }, positions: { where: { OR: [{ yesShares: { gt: 0 } }, { noShares: { gt: 0 } }], market: { status: { not: "DRAFT" } } } } } }, comments: { where: { status: "VISIBLE", market: { status: { not: "DRAFT" } } }, include: { market: { select: { slug: true, shortTitle: true } } }, orderBy: { createdAt: "desc" }, take: 10 } } });
  if (!user || !["USER", "ADMIN"].includes(user.role) || !user.profilePublic) notFound();
  const orderBookFills = await db.orderFill.count({ where: { OR: [{ makerOrder: { userId: user.id } }, { takerOrder: { userId: user.id } }] } });
