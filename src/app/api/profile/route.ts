...
export async function PATCH(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const profile = await runAuthenticatedMutation(request, user.id, (tx) => tx.user.update({ where: { id: user.id }, data, select: { username: true, displayName: true, bio: true, profilePublic: true } }));
