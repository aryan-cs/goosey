...
import { startWalletBrowserChain } from './lib/wallet-browser-localnet.ts';
const root = process.cwd();
mkdirSync(`${root}/output/playwright/wallet-funded`, { recursive: true });
const db = `${root}/output/playwright/wallet-funded/qa-${Date.now()}.db`;
...
try {
    chain = await startWalletBrowserChain({ walletAddress: sender, recipientAddress: recipient });
    if (interrupted)
    ...
    const env = {
        ...
        QA_RECIPIENT: recipient, QA_ALLOWANCE: String(chain.allowanceBaseUnits),
    ...
    browserProcess = spawn('node', ['scripts/wallet-browser-e2e-verify.mjs'], { env, stdio: 'inherit' });
    ...
    try {
        ...
        if (user.balanceMilli !== 0n || await audit.ledgerPosting.count() !== 0)
