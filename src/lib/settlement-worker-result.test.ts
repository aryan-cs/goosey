...
describe("settlement worker process result", () => {
  const healthy = { orderExpirationFailures: 0, marketCloseFailures: 0, failedRuns: 0 };
  it("accepts an entirely successful cycle", () => {
    expect(settlementWorkerCycleFailed(healthy)).toBe(false);
  });
  it.each(["orderExpirationFailures", "marketCloseFailures", "failedRuns"] as const)("rejects an isolated %s", (field) => {
