...
export function ProfileForm({ profile, privacyOnly = false }: { profile: Profile; privacyOnly?: boolean }) {
  ...
  return <form className="stacked-form" onSubmit={submit} aria-busy={busy}>
    {privacyOnly ? <>
      <label className="checkbox-field"><input type="checkbox" checked={draft.profilePublic} disabled={busy} onChange={(event) => update({ profilePublic: event.target.checked })} /><span><strong>Public profile</strong><small>Let people view your profile and identify you in recent trading activity. Market comments remain public either way.</small></span></label>
      <label className="checkbox-field"><input type="checkbox" checked={draft.leaderboardVisible} disabled={busy} onChange={(event) => update({ leaderboardVisible: event.target.checked })} /><span><strong>Appear on the leaderboard</strong><small>Show your username and prediction performance in public rankings.</small></span></label>
    </> : <>
      <label>Username<input name="username" autoComplete="username" value={draft.username} onChange={(event) => update({ username: event.target.value })} disabled={busy} minLength={3} maxLength={24} pattern="[a-zA-Z0-9_]+" required aria-describedby="settings-username-hint" /><small id="settings-username-hint" className="field-hint">3–24 letters, numbers, or underscores. Changing this also changes your public profile link.</small></label>
