...
export default async function CommunityPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="page-shell community-page">
    ...
    {comments.length ? <div className="community-feed">{comments.map((comment) => <article className="community-post" key={comment.id}>
      <header><span className="leader-avatar" aria-hidden="true">{initials(comment.user.displayName)}</span><div><UserProfileLink username={comment.user.username}><strong>{comment.user.displayName}</strong><small>@{comment.user.username}</small></UserProfileLink><small><time dateTime={comment.createdAt.toISOString()}>{comment.createdAt.toLocaleString("en-CA", { dateStyle: "medium", timeStyle: "short", timeZone: "America/Toronto" })}</time></small></div></header>
      <p>{comment.body}</p>
