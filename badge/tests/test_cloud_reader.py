...
snapshot = dict(generation='1234567890123456789', capturedAt='09/19 19:00 UTC', rangeStart=1000, rangeEnd=14401000,
                markets=[dict(slug='one',title='A market?',shortTitle='A market?',category='Hack the North',probability=50,
                              changeBps=100,volume='10',closes='09/20 18:30 UTC',status='OPEN',acceptingOrders=True,
                              history=[[50,1000],[51,2000]])])
frame = mailbox_frame(snapshot).decode()
parsed=reader(frame)
assert parsed['markets'][1]['history'][2][1] == 51
...
for bad in [frame+'junk\n',frame.replace('END\t123','END\t124'),frame.replace('\t5000\t100\t','\t10001\t100\t'),frame.replace('2000\t5100','500\t5100'),frame.replace('\t2\nH','\t33\nH'),frame.replace('GS2','GS3'),frame.replace('\t1\t2\nH','\tyes\t2\nH')]:
