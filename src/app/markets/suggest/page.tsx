...
export default async function SuggestMarketPage() {
  ...
  return <div className="page-shell narrow-page"><header className="page-header"><span className="eyebrow">Your idea</span><h1>Suggest a market</h1><p>Send us a question, the source we should check, and the exact YES condition.</p></header><SuggestionForm /><section className="moderation-panel" aria-labelledby="suggestion-history-heading"><div className="section-heading"><div><span className="eyebrow">Your suggestions</span><h2 id="suggestion-history-heading">Past suggestions</h2></div><span>{suggestions.length} total</span></div>{suggestions.length ? <div className="report-list">{suggestions.map((suggestion) => <article className="report-item" key={suggestion.id}><header><strong>{suggestion.title}</strong><span>{suggestion.status.toLowerCase()}</span></header><p>{suggestion.description}</p>{suggestion.reviewNote && <small>Organizer note: {suggestion.reviewNote}</small>}<footer><span>{suggestion.category} · sent {suggestion.createdAt.toLocaleDateString("en-CA", { dateStyle: "medium" })}</span>{suggestion.market && <Link href={`/markets/${suggestion.market.slug}`}>View {suggestion.market.shortTitle}</Link>}</footer></article>)}</div> : <p className="muted-copy">You have not suggested a market yet.</p>}</section></div>;
}
