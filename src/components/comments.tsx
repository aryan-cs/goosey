...
export function CommentSection({ marketId, currentUserId, csrfToken, endpoint, maxLength = 800 }: CommentSectionProps) {
  ...
  return (
    <section className="comments-section" aria-labelledby="discussion-heading">
      ...
      <div className="comment-toolbar"><div className="segmented compact" role="group" aria-label="Sort comments"><button type="button" aria-pressed={sort === "top"} className={sort === "top" ? "active" : ""} onClick={() => { setLoading(true); setError(null); setSort("top"); }}>Most replies</button><button type="button" aria-pressed={sort === "newest"} className={sort === "newest" ? "active" : ""} onClick={() => { setLoading(true); setError(null); setSort("newest"); }}>Newest</button></div><p>Keep it friendly and back up your take.</p></div>
