...
type Review = { intent: Intent; transaction: PreparedWalletTransaction; digest: string; fee: bigint; rent: bigint; facts: [string,string][] };
...
function MarketAccount({ runtime, wallet, snapshot, marketId }: WalletAccountProps & { marketId: bigint }) {
  ...
  async function prepare(intent: Intent) {
    ...
    try {
      ...
      else { transaction = await prepareResolutionClaim({ runtime, payer: sender, targetWallet: address(account), marketId, signal }); facts.push(["Proceeds", "Credit this market’s escrow; withdraw separately"]); }
      ...
      unchanged(signal); setView(latest); setReview({ intent, transaction, digest: latest.digest, fee: fee.value, rent, facts });
      ...
      <div className={styles.layout}>
        <div className={styles.column}>
          ...
          <section className={shared.panel}><h2>Your positions</h2>{seat ? <div className={styles.metrics}>{[["Available feathers",feathers(seat.availableCash)],["Reserved feathers",feathers(seat.reservedCash)],["YES contracts",seat.yes.toString()],["NO contracts",seat.no.toString()],["Reserved YES",seat.reservedYes.toString()],["Reserved NO",seat.reservedNo.toString()]].map(([label,value])=><div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div> : <p>No market seat registered for this wallet.</p>}
            ...
            {seat && data.resolution.phase === 3 && <button className="button button-primary" disabled={disabled || Boolean(review)} onClick={()=>void prepare({kind:"claim"})}>Review resolution claim</button>}
          </section>
