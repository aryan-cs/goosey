...
local function readCloudFrame(data)
  ...
  for line in data:gmatch("([^\n]*)\n") do
    if #lines>=114 or #line>600 then return nil end
    local parts={}
  ...
  local cursor,seen,totalPoints=2,{},0
  for i=1,count do
    local row=lines[cursor];cursor=cursor+1
    ...
    for j=1,n do
      local point=lines[cursor];cursor=cursor+1
  ...
  local ending=lines[cursor]
  if #lines~=cursor or not ending or #ending~=2 or ending[1]~="END" or ending[2]~=head[2] then return nil end
  return result
end
