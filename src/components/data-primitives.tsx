...
export function LeaderboardRow({ user, current = false }: { user: LeaderboardUser; current?: boolean }) {
  ...
  return <LeaderboardIdentity user={user} className={`leaderboard-row${current ? " current-user" : ""}`} label={`${user.displayName}, rank ${user.rank}, ${user.score.toLocaleString(undefined, { maximumFractionDigits: 0 })} feathers total${current ? ", your position" : ""}`}><span className="rank" aria-label={`Rank ${user.rank}`}>{user.rank === 1 ? <span aria-hidden="true"><Crown /></span> : user.rank}</span><span className="leader-avatar" aria-hidden="true">{initials(user.displayName)}</span><span className="leader-name"><strong>{user.displayName}</strong>{user.availableBalance === undefined && <small>{user.badge ?? `${user.marketsTraded ?? 0} markets`}</small>}</span>{movementLabel && <span className={user.movement! >= 0 ? "rank-up" : "rank-down"} aria-label={movementLabel}><span aria-hidden="true">{user.movement! > 0 ? "↑" : user.movement! < 0 ? "↓" : "0"} {Math.abs(user.movement!)}</span></span>}<span className="leader-score"><FeatherIcon /> <strong>{user.score.toLocaleString(undefined, { maximumFractionDigits: 0 })}<span className="sr-only"> feathers total</span></strong></span></LeaderboardIdentity>;
}

export function LeaderboardPodium({ users }: { users: LeaderboardUser[] }) {
  return <div className="leaderboard-podium" aria-label="Top forecasters">{users.map((user) => <LeaderboardIdentity id={`player-${user.id}`} user={user} className={`podium-place podium-${user.rank}`} label={`${user.displayName}, rank ${user.rank}, ${user.score.toLocaleString(undefined, { maximumFractionDigits: 0 })} feathers total`} key={user.id}><span className="podium-icon" aria-hidden="true">{user.rank === 1 ? <Trophy /> : <span className="rank">{user.rank}</span>}</span><div className="leader-avatar" aria-hidden="true">{initials(user.displayName)}</div><strong>{user.displayName}</strong><span aria-hidden="true"><FeatherIcon /> {user.score.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></LeaderboardIdentity>)}</div>;
}
