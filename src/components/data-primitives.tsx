...
export function LeaderboardRow({ user, current = false }: { user: LeaderboardUser; current?: boolean }) {
  ...
  return <Link className={`leaderboard-row${current ? " current-user" : ""}`} href={`/users/${encodeURIComponent(user.username)}`} aria-label={`View ${user.displayName}'s profile, rank ${user.rank}, ${user.score.toLocaleString()} feathers${current ? ", your position" : ""}`}><span className="rank" aria-label={`Rank ${user.rank}`}>{user.rank === 1 ? <span aria-hidden="true"><Crown /></span> : user.rank}</span><span className="leader-avatar" aria-hidden="true">{user.displayName.slice(0, 2).toUpperCase()}</span><span className="leader-name"><strong>{user.displayName}</strong><small>{user.badge ?? `${user.marketsTraded ?? 0} markets`}</small></span>{movementLabel && <span className={user.movement! >= 0 ? "rank-up" : "rank-down"} aria-label={movementLabel}><span aria-hidden="true">{user.movement! > 0 ? "↑" : user.movement! < 0 ? "↓" : "0"} {Math.abs(user.movement!)}</span></span>}<span className="leader-score"><FeatherIcon /> <strong>{user.score.toLocaleString()}<span className="sr-only"> feathers</span></strong></span></Link>;
}

export function LeaderboardPodium({ users }: { users: LeaderboardUser[] }) {
  return <div className="leaderboard-podium" aria-label="Top forecasters">{users.map((user) => <Link className={`podium-place podium-${user.rank}`} href={`/users/${encodeURIComponent(user.username)}`} aria-label={`${user.displayName}, rank ${user.rank}, ${user.score.toLocaleString()} feathers`} key={user.id}><span className="podium-icon" aria-hidden="true">{user.rank === 1 ? <Trophy /> : <span className="rank">{user.rank}</span>}</span><div className="leader-avatar" aria-hidden="true">{user.displayName.slice(0, 2).toUpperCase()}</div><strong>{user.displayName}</strong><span aria-hidden="true">#{user.rank} · <FeatherIcon /> {user.score.toLocaleString()}</span></Link>)}</div>;
}
