...
type Profile = { username: string; bio: string; profilePublic: boolean };
export function ProfileForm({ profile, privacyOnly = false }: { profile: Profile; privacyOnly?: boolean }) {
  ...
  const dirty = privacyOnly ? draft.profilePublic !== saved.profilePublic : draft.username !== saved.username || draft.bio !== saved.bio;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = privacyOnly ? { profilePublic: draft.profilePublic } : { username: draft.username, bio: draft.bio };
    try {
  ...
  return <form className="stacked-form" onSubmit={submit} aria-busy={busy}>
    {privacyOnly ? <>
      <p>All active players appear on the leaderboard. Your name and ranking are public and cannot be hidden.</p>
      <label className="checkbox-field"><input type="checkbox" checked={draft.profilePublic} disabled={busy} onChange={(event) => update({ profilePublic: event.target.checked })} /><span><strong>Show community details</strong><small>Your trading profile is always visible. Turn this on to also show your bio and include your account in people search.</small></span></label>
    </> : <>
